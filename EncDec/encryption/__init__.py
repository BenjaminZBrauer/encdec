...
# ANY CODE IS THIS BLOCK WILL BE OVERWRITTEN.

def getEncFunctions():
	return [lambda x: Invert(x), lambda x: toHex(x), lambda x: Invert(x), 
		lambda x: firstInversion(x), lambda x: snake(x), lambda x: firstInversion(x), 
		lambda x: firstInversion(x)]

def getDecFunctions():
	return [lambda x: rootPosition(x), lambda x: rootPosition(x), lambda x: unSnake(x), 
		lambda x: rootPosition(x), lambda x: Invert(x), lambda x: toString(x), 
		lambda x: Invert(x)]
